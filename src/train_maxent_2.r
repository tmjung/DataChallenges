# =============================================================================
# Train MaxEnt model with coordinates and yearly climate predictors
...
raw <- read_csv(
  ...
  col_types = cols(
    ...
    Niederschlag_Jahr = col_character(),
    Sonnenstunden_Jahr = col_character(),
...
raw <- raw %>%
  mutate(
    ...
    Niederschlag_Jahr = as.numeric(Niederschlag_Jahr),
    Sonnenstunden_Jahr = as.numeric(Sonnenstunden_Jahr),
    Temperatur_Jahr = as.numeric(Temperatur_Jahr)
  )

presence_df <- raw %>%
  filter(
    !is.na(lng), !is.na(lat),
    !is.na(Niederschlag_Jahr), !is.na(Sonnenstunden_Jahr), !is.na(Temperatur_Jahr)
  ) %>%
  select(lng, lat, Niederschlag_Jahr, Sonnenstunden_Jahr, Temperatur_Jahr)
...
# 3. Generate background points and sample environmental values -------------
set.seed(42)
...
# Use the observed climate variable distribution to assign background values.
background_env <- presence_df %>%
  slice_sample(n = nrow(absence_df), replace = TRUE) %>%
  select(Niederschlag_Jahr, Sonnenstunden_Jahr, Temperatur_Jahr)
absence_df <- bind_cols(absence_df, background_env)

cat("Background points generated:", nrow(absence_df), "\n")

# 4. Optional visualization clustering -------------------------------------
...
make_predictors <- function(df) {
  df %>%
    ...
    select(lon, lat, lon2, lat2, lon_lat,
           Niederschlag_Jahr, Sonnenstunden_Jahr, Temperatur_Jahr)
}
...
# 7. Train MaxEnt-like model ------------------------------------------------
cat("Training maxnet model...\n")
mx <- maxnet::maxnet(p = train_p, data = train_data)
cat("Model trained.\n")

# 8. Create prediction grid and predict ------------------------------------
# Environmental variables are set to mean values across observations,
# because spatial rasters are not available in this simple example.
env_means <- presence_df %>%
  summarize(
    Niederschlag_Jahr = mean(Niederschlag_Jahr, na.rm = TRUE),
    Sonnenstunden_Jahr = mean(Sonnenstunden_Jahr, na.rm = TRUE),
    Temperatur_Jahr = mean(Temperatur_Jahr, na.rm = TRUE)
  )
...
grid <- expand.grid(lng = lon_seq, lat = lat_seq) %>%
  mutate(
    Niederschlag_Jahr = env_means$Niederschlag_Jahr,
    Sonnenstunden_Jahr = env_means$Sonnenstunden_Jahr,
    Temperatur_Jahr = env_means$Temperatur_Jahr
  )
grid_preds <- make_predictors(grid)
grid$pred <- predict(mx, newdata = grid_preds, type = "logistic")

# 9. Convert grid to raster and mask to Bavaria -----------------------------
...
p <- ggplot() +
  ...
  labs(title = "MaxEnt-Prediction mit Klimadaten", x = "Längengrad", y = "Breitengrad", fill = "Eignung") +
  theme_minimal()

print(p)
ggsave("train_maxent_2_map.png", plot = p, width = 10, height = 8, dpi = 300)
cat("Map saved: train_maxent_2_map.png\n")

# 11. Save model to disk ----------------------------------------------------
saveRDS(mx, file = "train_maxent_2_model.rds")
cat("Model saved: train_maxent_2_model.rds\n")
