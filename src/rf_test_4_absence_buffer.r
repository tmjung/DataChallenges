...
cat("OOB Brier score:", rf_model$prediction.error, "\n")
...
oob_accuracy <- mean(oob_pred_class == train_df$presence)
...
inside_bavaria <- lengths(st_intersects(prob_sf, bavaria_sf)) > 0
prob_df <- prob_df[inside_bavaria, ]
