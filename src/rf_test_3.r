...
cat("Candidate absence points:", nrow(absence_candidates), "\n")

# Abstand in Metern berechnen
presence_m <- st_transform(presence_sf, 25832)
absence_m  <- st_transform(absence_candidates, 25832)

dist_mat <- st_distance(absence_m, presence_m)
min_dist_m <- apply(as.matrix(dist_mat), 1, min)

selected <- which(min_dist_m > 5000)
selected <- selected[seq_len(min(length(selected), n_absence))]

absence_sf <- absence_candidates[selected, , drop = FALSE]
...
writeRaster(
  probability_raster,
  "rf_5_probability.tif",
  overwrite = TRUE
)

cat("Raster saved: rf_5_probability.tif\n")
...
cat("Map saved: rf_5_heatmap.png\n")
...
cat("Map saved: rf_5_heatmap_wp.png\n")

# -------------------------------------------------------------------------
# 13. Modell und Trainingsdaten speichern
# -------------------------------------------------------------------------

saveRDS(rf_model, "rf_5_model.rds")
write_csv(train_df, "rf_5_training_data.csv")

cat("\nDone.\n")
cat("Created:\n")
cat(" - rf_5_probability.tif\n")
cat(" - rf_5_heatmap.png\n")
cat(" - rf_5_heatmap_wp.png\n")
cat(" - rf_5_model.rds\n")
cat(" - rf_5_training_data.csv\n")